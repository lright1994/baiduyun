...
class PCSBase(object):
    ...
    def __init__(self, username, password, captcha_func=None,
                 verify_func=None):
    ...
    def user_info(self, **kwargs):
        ...
        return self._request(
            'membership/user', 'user', url=url, extra_params=params, **kwargs)
    ...
    def _login(self):
        ...
        password_rsaed = base64.b64encode(
        ...
        while True:
            ...
            if b'err_no=257' in result.content or b'err_no=6' in result.content:
                code_string = re.findall(b'codeString=(.*?)&',
                                         result.content)[0]
                self.codeString = code_string
                logging.debug('need captcha, codeString=' + code_string.decode(
                    'utf-8'))
        ...
        if data or files:
            ...
            if data:
                ...
                response = self.session.post(
                    api, data=data, verify=False, headers=headers, **kwargs)
            else:
...
class PCS(PCSBase):
    ...
    def upload(self,
        ...
        params = {'dest_dir': dest_dir, 'ondup': ondup, 'filename': filename}
    ...
    def _locatedownload(self, remote_path, **kwargs):
        ...
        url = 'https://{0}/rest/2.0/pcs/file'.format(BAIDUPCS_SERVER)
        return self._request(
            'file', 'locatedownload', url=url, extra_params=params, **kwargs)
    ...
    def download_url(self, remote_path, **kwargs):
        ...
        if jdata['errno'] != 0:
            jdata = self.__err_handler(
                'generic', jdata['errno'], self.meta, args=(remote_path, ))
    ...
    def share_dlink_for_fs_ids(self, fsid_list, shareid, uk, sign):
        ...
        sekey = json.dumps({
            "sekey": urlparse.unquote(self.session.cookies["BDCLND"])
        })
        data = {
            "encrypt": 0,
            "extra": sekey,
        ...
        reqheader = {
            "Referer":
            "https://pan.baidu.com/share/link?shareid=" + shareid + "&uk=" + uk
        }
        resp = self._request(
            None,
            data=data,
    ...
    def _handle_shared_captcha(self, shareid, uk, password):  # 处理分享页的验证码
        rep = json.loads(
            self._request(
                None,
                data={"prod": "shareverify"},
                url="https://pan.baidu.com/api/getcaptcha").content.decode(
                    'utf-8'))

        vcode = self.captcha_func("https://pan.baidu.com/genimage?" + str(rep[
            "vcode_str"]))
        return self._verify_shared_file(
            shareid,
            uk,
            password,
            vcode=vcode,
            vcode_str=str(rep["vcode_str"]))
    ...
    def _scan_folder(self,
        ...
        if init_file_list:
            ...
            for item in filelist:
                filelistcurrentdict[item[
                    "server_filename"]] = self._scan_folder(
                        shareid, uk, item["server_filename"])

        else:
            filelist = self.list_shared_folder(
                shareid, uk, dir_path, page=1, number=100)['list']
        ...
        for info in filelist:
            ...
            if int(info['isdir']) == 1:
                filelistcurrentdict[info['path']] = self._scan_folder(
                    shareid, uk, parent_path + info['path'])
            else:
    ...
    def prepare_url(self, url, password=None, filter_callback=None):
        ...
        html = self._request(
            None,
            url="https://pan.baidu.com/share/link?shareid=" + str(shareid) +
            "&uk=" + str(uk)).content
        ...
        if pwindex > -1:  # 需要密码
            if password:
                errno = int(
                    self._verify_shared_file(
                        shareid, uk,
                        password)["errno"])  # 0正常 -12验证码错误 -9提取码错误
                captcha_tried = False
                ...
                while 1:
                    if errno == 0:
                        ...
                        # 需要验证码
                        errno = int(
                            self._handle_shared_captcha(shareid, uk, password)[
                                "errno"])
                        captcha_tried = True
                    elif errno == -9:
        ...
        return self._download_shared_list(
            shareid, uk, filter_callback=filter_callback)

    def _download_shared_list(self,
                              shareid,
                              uk,
                              initial_path="/",
                              filter_callback=None):
        ...
        initurl = "https://pan.baidu.com/share/link?shareid=" + str(
            shareid) + "&uk=" + str(uk)
        html = self._request(None, url=initurl).content
        r = re.compile(
            r".*yunData.setData\((\{\"loginstate.*\})\);.*")  # 搜索第二类context
        m = r.search(html)
        if m:
            ...
            try:
                ...
                raise ValueError("File_List Not Found in HTML from: " +
                                 initurl)
            for item in fl:
                if int(item["isdir"]) == 1:
                    downloadurllist[item[
                        "server_filename"]] = self._scan_folder(
                            shareid, uk,
            ...
            raise ValueError("yunData.setData Not Found in HTML from: " +
                             initurl)
    ...
    def save_share_list(self, url, path, password=None, filter_callback=None):
        ...
        if m:
            ...
            save_share_file_ret = self._save_shared_file_list(
                shareid, uk, path, ret['filelist'])
            ...
            return {
                "errno": -1,
                "error_msg":
                "PCS.save_share_list failed, mayby url is incorrect!"
            }
    ...
    def download(self, remote_path, **kwargs):
        ...
        url = 'https://{0}/rest/2.0/pcs/file'.format(BAIDUPCS_SERVER)
        return self._request(
            'file', 'download', url=url, extra_params=params, **kwargs)
    ...
    def get_streaming(self, path, stype="M3U8_AUTO_480", **kwargs):
        ...
        while True:
            ret = self._request(
                'file', 'streaming', url=url, extra_params=params, **kwargs)
            if not ret.ok:
                logging.debug('get_streaming ret_status_code %s' %
                              ret.status_code)
    ...
    def delete(self, path_list, **kwargs):
        ...
        return self._request(
            'filemanager', 'delete', url=url, data=data, **kwargs)
    ...
    def list_streams(self,
        ...
        return self._request(
            'categorylist', 'list', url=url, extra_params=params, **kwargs)
    ...
    def add_download_task(self,
        ...
        if source_url.startswith('magnet:?'):
            ...
            return self.add_magnet_task(source_url, remote_path, selected_idx,
                                        **kwargs)
        elif source_url.endswith('.torrent'):
            print('BitTorrent: "%s"' % source_url)
            return self.add_torrent_task(source_url, remote_path, selected_idx,
                                         **kwargs)
        else:
            ...
            url = 'http://{0}/rest/2.0/services/cloud_dl'.format(
                BAIDUPAN_SERVER)
            return self._request(
                'services/cloud_dl', 'add_task', url=url, data=data, **kwargs)

    def add_torrent_task(self,
                         torrent_path,
                         save_path='/',
                         selected_idx=(),
                         **kwargs):
        ...
        if isinstance(selected_idx, (tuple, list, set)):
            if len(selected_idx) > 0:
                ...
                selected_idx = ','.join(
                    map(str,
                        range(1, len(response['torrent_info']['file_info']) +
                              1)))
        else:
            selected_idx = ''
        ...
        return self._request(
            'create', 'add_task', url=url, data=data, **kwargs)
    ...
    def _get_torrent_info(self, torrent_path):
        ...
        return self._request(
            'cloud_dl', 'query_sinfo', url=url, data=data, timeout=30)
    ...
    def add_magnet_task(self, magnet, remote_path, selected_idx=(), **kwargs):
        ...
        if isinstance(selected_idx, (tuple, list, set)):
            if len(selected_idx) > 0:
                ...
                selected_idx = ','.join(
                    map(str, range(1, len(response['magnet_info']) + 1)))
        else:
            selected_idx = ''
        ...
        return self._request(
            'create', 'add_task', url=url, data=data, timeout=30)
    ...
    def _get_magnet_info(self, magnet):
        ...
        return self._request(
            'cloud_dl', 'query_magnetinfo', url=url, data=data, timeout=30)
    ...
    def cancel_download_task(self, task_id, expires=None, **kwargs):
        ...
        url = 'http://{0}/rest/2.0/services/cloud_dl'.format(BAIDUPAN_SERVER)
        return self._request(
            'services/cloud_dl', 'cancel_task', url=url, data=data, **kwargs)
    ...
    def list_recycle_bin(self,
        ...
        return self._request(
            'recycle', 'list', url=url, extra_params=params, **kwargs)
    ...
    def thumbnail(self, path, height, width, quality=100, **kwargs):
        ...
        return self._request(
            'thumbnail', 'generate', url=url, extra_params=params, **kwargs)
    ...
    def meta(self, file_list, **kwargs):
        ...
        return self._request(
            'filemetas?blocks=0&dlink=1', 'filemetas', data=data, **kwargs)
